  ...
  for ( int i = 0; i < th_tile_count; i++ )
    {
      ...
      const float y0 = -40; //-0.01 - platform_xrad * sin(theta0); project
      const float y1 = -40; //-0.01 - platform_xrad * sin(theta1);
  ...
  for ( Ball *ball; balls_iterate(ball); )
    {
      ...
      if ( pos.y <= -40 )
      {
          ...
          //
          pCoor tact(pos.x, -40, pos.z); //pos.x > platform_xmid ? platform_xmax : platform_xmin
